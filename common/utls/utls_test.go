...
func TestNewUTLSRoundTripperCase(t *testing.T) {
	mixed, err := NewUTLSRoundTripper("HelloFirefox_Auto", nil, nil, nil)
	if err != nil {
		t.Fatalf("error on %q: %v", "HelloFirefox_Auto", err)
	}
	upper, err := NewUTLSRoundTripper("HELLOFIREFOX_AUTO", nil, nil, nil)
	if err != nil {
		t.Fatalf("error on %q: %v", "HELLOFIREFOX_AUTO", err)
	}
	lower, err := NewUTLSRoundTripper("hellofirefox_auto", nil, nil, nil)
	if err != nil {
		t.Fatalf("error on %q: %v", "hellofirefox_auto", err)
	}
...
func TestUTLSClientHello(t *testing.T) {
	Convey("UTLSRoundTripper", t, func() {
		Convey("When using a modified TLS Client Hello", func() {
			...
			rt, err := NewUTLSRoundTripper("HelloIOS_11_1", &utls.Config{InsecureSkipVerify: true, ServerName: "localhost"}, nil, nil)
			if err != nil {
			...
			So(buf, ShouldHaveLength, len(pattern))
			for i := 0; i < len(pattern); i++ {
				a := buf[i]
				b := pattern[i]
				if b != '.' {
					Convey(fmt.Sprintf("The fingerprint in position: %v should match", i), func() {
						So(a, ShouldEqual, b)
					})
				}
			}
...
func TestUTLSServerName(t *testing.T) {
	Convey("UTLSRoundTripper", t, func() {
		Convey("When using various server name options", func() {
			...
			rt, err := NewUTLSRoundTripper(clientHelloIDName, &utls.Config{InsecureSkipVerify: true}, nil, nil)
			if err != nil {
				So(err, ShouldBeNil)
			}
			buf, err := clientHelloResultingFromRoundTrip(t, "127.0.0.1", rt.(*UTLSRoundTripper))
			...
			Convey("Given no ServerName and hostname dial, server_name extension should come from the dial address", func() {
				rt, err = NewUTLSRoundTripper(clientHelloIDName, &utls.Config{InsecureSkipVerify: true}, nil, nil)
				if err != nil {
					So(err, ShouldBeNil)
				}
				...
				So(buf, shouldContainBytes, []byte("\x00\x00\x00\x0e\x00\x0c\x00\x00\x09localhost"))
			})

			Convey("Given ServerName, dial IP address server_name extension should come from the ServerName", func() {
				rt, err = NewUTLSRoundTripper(clientHelloIDName, &utls.Config{InsecureSkipVerify: true, ServerName: "test.example"}, nil, nil)
				if err != nil {
					So(err, ShouldBeNil)
				}
				buf, err = clientHelloResultingFromRoundTrip(t, "127.0.0.1", rt.(*UTLSRoundTripper))
				...
				So(buf, shouldContainBytes, []byte("\x00\x00\x00\x11\x00\x0f\x00\x00\x0ctest.example"))
			})

			Convey("Given ServerName, dial hostname server_name extension should from the ServerName", func() {
				rt, err = NewUTLSRoundTripper(clientHelloIDName, &utls.Config{InsecureSkipVerify: true, ServerName: "test.example"}, nil, nil)
				if err != nil {
					So(err, ShouldBeNil)
				}
...
func TestUTLSHTTPWithProxy(t *testing.T) {
	Convey("UTLSRoundTripper with Proxy", t, func() {
		...
		for _, proxyURL := range []url.URL{
			...
			Convey(fmt.Sprintf("With %v proxy", proxyURL.Scheme), func() {
				rt, err := NewUTLSRoundTripper("HelloFirefox_63", &utls.Config{InsecureSkipVerify: true}, &proxyURL, nil)
				if err != nil {
					So(err, ShouldBeNil)
				}
