...
// Adapted with small changes from https://gitweb.torproject.org/pluggable-transports/meek.git/tree/meek-client/utls.go
//
...
// Default RoundTripper to make all our requests when utls is not in effect.
// We use the defaults, except we take control of the Proxy
...
type UTLSRoundTripper struct {
	...
	// A default http.Transport that can be passed in to set coniguration settings with
	defaultTransport *http.Transport
}

func (urt *UTLSRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	switch req.URL.Scheme {
	case "http":
		// If http, we don't invoke uTLS; just pass it to an ordinary http.Transport.
		return urt.httpRT.RoundTrip(req)
	case "https":
	default:
	...
	if urt.rt == nil {
		...
		// as appropriate.
		var err error
		urt.rt, err = urt.makeRoundTripper(req.URL, urt.clientHelloID, urt.config, urt.proxyDialer)
		if err != nil {
			return nil, err
		}
	}
...
func (urt *UTLSRoundTripper) makeRoundTripper(url *url.URL, clientHelloID *utls.ClientHelloID, cfg *utls.Config, proxyDialer proxy.Dialer) (http.RoundTripper, error) {
	...
	default:
		...
		// urt.defaultTransport, such as TLSHandshakeTimeout and
		// IdleConnTimeout, before overriding DialTLS.
		tr := urt.defaultTransport.Clone()
		tr.DialTLS = dialTLS
		return tr, nil
	}
}
...
func NewUTLSRoundTripper(name string, cfg *utls.Config, proxyURL *url.URL, defaultTransport *http.Transport) (http.RoundTripper, error) {
